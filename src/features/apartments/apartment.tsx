		...
		<>
			...
			<main className="flex w-full flex-col items-center justify-center px-4 text-secondary-100 lg:px-0">
				<section className="flex w-full max-w-[1200px] flex-col gap-5 py-10">
					<div className="mb-10 flex w-full flex-col justify-between lg:flex-row lg:items-center">
						...
						<div className="grid w-full grid-cols-1 gap-5 lg:grid-cols-3">
							{data.data.imageUrls
								.map((url, index) => (
									<div
										key={index}
										className={`relative aspect-[3/2] rounded-lg ${index === 0 ? "lg:col-span-2" : "w-full"}`}>
										<Image
											src={url}
											alt="logo"
											fill
					...
					<div className="flex w-full flex-col items-start gap-4">
						...
						<div className="w-full">
							<p className="text-sm lg:text-lg">{data.data.description}</p>
						</div>
					</div>
					...
					<div className="flex w-full flex-col items-start gap-4">
						...
						<div className="flex w-full flex-wrap items-center gap-4">
							<span className="rounded-full bg-secondary-200 px-4 py-2 text-sm text-accent-200 lg:text-lg">
					...
					<div className="flex w-full flex-col items-start gap-4">
						...
						<div className="flex w-full flex-wrap items-center gap-4">
							{data.data.amenities.map((amenity, index) => (
								<span
									key={index}
									className="rounded-full bg-secondary-200 px-4 py-2 text-sm text-accent-200 lg:text-lg">
									{amenity}
								</span>
							))}
						</div>
